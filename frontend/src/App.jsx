...
import { ResolutionPage1, ResolutionPage2 } from './components/ResolutionContent';
...
const App = () => {
  ...
  const fetchLatestSignatures = useCallback(async () => {
    ...
    try {
      ...
      setLastSyncError('Could not load signatures from server. Please ensure the backend is running and MongoDB is connected.');
    }
  }, [isBlocked]);

  useEffect(() => {
    // Defer to avoid running state updates synchronously in effect body (eslint rule).
    ...
    <div className="document-container">
      ...
      <ResolutionPage2>
        ...
        <SignatureDialog
          ...
          title={
            activeSigIndex !== null
              ? `Signature - ${shareholders[activeSigIndex] || `#${activeSigIndex + 1}`}`
              : 'Signature'
          }
          existingDataUrl={activeSigIndex !== null ? signatures?.[String(activeSigIndex)] : ''}
          disabled={isBlocked || isSubmitted}
          saving={activeSigIndex !== null ? !!sigSavingStates[String(activeSigIndex)] : false}
          onClose={closeSignatureDialog}
          onSave={(dataUrl) => {
            if (activeSigIndex === null) return;
            saveSignatureForIndex(activeSigIndex, dataUrl);
          }}
        />
        ...
        {!isSubmitted ? (
          ...
          <div className="submitted-banner" style={{ marginTop: '20px', textAlign: 'center', background: '#f8fff8', border: '1px solid #d4edda', padding: '15px', borderRadius: '8px' }}>
            <div style={{ color: '#155724', fontWeight: 'bold', marginBottom: '5px' }}>✅ RESOLUTION SUBMITTED</div>
            <div style={{ fontSize: '10pt', color: '#155724', marginBottom: '15px' }}>{submissionDate}</div>
